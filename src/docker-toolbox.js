...
var machineExec = function (args, cb) {
    ...
    if (process.platform.toLowerCase() === 'linux') {
        ...
        // OSX
        var proc = childProcess.spawn('docker-machine', scope.args);
...
// Inspects scope and returns the first available docker machine name
var getDefaultMachine = function (scope) {
    var retVal = '';
    if (scope && scope.conf && scope.conf.docker && scope.conf.docker.machine) {
        var obj = scope.conf.docker.machine;
        retVal = obj[Object.keys(obj)[0]].tld;
    }
    return retVal;
};
...
var D = {
    ...
    'machine': {
        ...
        start: function (cb) {
            ...
            // Setup arguments to `docker-machine start`    
            scope.args.unshift("start");
            
            if (scope.args.length < 2) {
                // User did not supply an optional machine name, use first available machine name from config file
                // to-do: inspect docker directly to get first available machine rather than use config file
                scope.args.push(getDefaultMachine(scope));
            }

            machineExec(scope.args, cb);
        },
        stop: function (cb) {
            // Setup arguments to `docker-machine stop`    
            scope.args.unshift("stop");
            
            if (scope.args.length < 2) {
                // User did not supply an optional machine name, use first available machine name from config file
                // to-do: inspect docker directly to get first available machine rather than use config file
                // to-do: if no machine is specified should probably iterate and stop all machines - or could that be fatal for production endpoints? @sean let's discuss
                scope.args.push(getDefaultMachine(scope));
            }
